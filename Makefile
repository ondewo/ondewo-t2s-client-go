...
# =====================================================================================
# ondewo-t2s-client-go - Makefile
#
# The ONDEWO T2S (Text-to-Speech) gRPC client for go. Everything below the
# `api/` directory is GENERATED from the .proto files of the ondewo-t2s-api submodule by the
...
#   make test                     # go test ./...
#
# Versioning: ONDEWO_T2S_VERSION (below) is the single source of truth and MUST
# match the ONDEWO T2S API in major and minor version.
#
...
GO_MODULE_BASE_PATH=github.com/ondewo/ondewo-t2s-client-go
GO_MODULE_MAJOR=$(firstword $(subst ., ,$(ONDEWO_T2S_VERSION)))
...
GO_RELEASE_TAG=v${ONDEWO_T2S_VERSION}
...
CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO T2S Go Client ${ONDEWO_T2S_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-t2s-client-go"
...
install_precommit_hooks: ## Installs pre-commit hooks and sets them up for the ondewo-t2s-client-go repo
	pre-commit install
...
print_version: ## Print ONDEWO_T2S_VERSION and nothing else
	@echo "${ONDEWO_T2S_VERSION}"
...
generate_ondewo_protos: ## Generate the go stubs from the .proto files of the ondewo-t2s-api submodule
	@[ -d ${ONDEWO_PROTOS_DIR} ] || { \
...
# stays invisible to `go get` once the release is tagged ("module .../ondewo-t2s-client-go@v6.6.0:
...
check_go_module_path: ## Assert go.mod and every import agree with the /vN suffix ONDEWO_T2S_VERSION requires
	@declared=`sed -n 's|^module[[:space:]][[:space:]]*\([^[:space:]]*\).*|\1|p' go.mod | head -1`; \
	if [ "$$declared" != "${GO_MODULE_PATH}" ]; then \
		echo "$(RED)[ERROR]$(NC) go.mod declares module '$$declared', but ONDEWO_T2S_VERSION=${ONDEWO_T2S_VERSION} requires '${GO_MODULE_PATH}'"; \
	...
	echo "$(GREEN)[SUCCESS]$(NC) go.mod declares $$declared, matching ONDEWO_T2S_VERSION=${ONDEWO_T2S_VERSION}"
...
checkout_defined_submodule_versions: ## Check out the submodule versions pinned in the Variables chapter
	...
	git -C ${ONDEWO_API_DIR} checkout ${ONDEWO_T2S_API_GIT_BRANCH}
...
# only noticed by whoever reads the release page afterwards.
	...
	if [ -z "$$notes" ]; then \
		echo "$(RED)[ERROR]$(NC) RELEASE.md has no '## Release ONDEWO T2S Go Client ${ONDEWO_T2S_VERSION}' entry"; \
