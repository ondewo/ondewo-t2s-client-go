...
> import path of release `7.1.2` is `github.com/ondewo/ondewo-t2s-client-go/v7/api/ondewo/t2s`. The
...
func main() {
    ...
    bearerToken := os.Getenv("ONDEWO_T2S_ACCESS_TOKEN")
    ...
    // api/ondewo/t2s/ for the ones this product exposes.
    client := t2spb.NewExampleServiceClient(conn)

    response, err := client.ExampleMethod(ctx, &t2spb.ExampleRequest{})
    if err != nil {
